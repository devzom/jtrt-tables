var JTrtEditor = function (tableContainer) {
  ...
  this.loader = jQuery("#loaderIco")
    .detach()
    .hide()
    .appendTo("body")
    .css("height", document.documentElement.scrollHeight + 121);
  ...
  this.tableSettings = {
    ...
    afterOnCellMouseDown: function (event, location, smth) {
      ...
      jtrt_toolbar_value_input.val(
        jQuery(Iam.handsOnTab.getCell(location["row"], location["col"])).html(),
      ); // set the value of the input box to equal the first selected cell's data'
      jtrt_toolbar_value_input.data("editting-row", location["row"]); // add helper data attributes so I can do more editting later for when the input box is changed
      jtrt_toolbar_value_input.data("editting-col", location["col"]); // same as above jeez

      // Get the active editor for handsontable so I can unescape the html characters when they double click to edit
      var activeeditor = Iam.handsOnTab
        .getCellMeta(location["row"], location["col"])
...
JTrtEditor.prototype.init = function () {
  ...
  jQuery(".jtedittexttoolbar").on("click", function () {
    Iam.editCellText(
      jQuery(this).data("jtrt-toolbar-opt"),
      jQuery(this).data("jtrt-toolbar-opt-val"),
    );
  });

  jQuery(document).on(
    "keypress",
    "#jteditfont ul input[type='number'],#jtinsertlink ul input[type='text'],#jtinputvalbox,.jtcoloreditpicker,#jtlink",
    function (event) {
  ...
  jQuery(".jtrt-toolbar-more").on("click", function (event) {
    ...
    if (selected != undefined && selected.length > 0) {
      if (jQuery(this).attr("id") == "jtinsertlink") {
        var curlink = "";
        var curval = Iam.decodeHtml(
          Iam.handsOnTab.getDataAtCell(selected[0], selected[1]),
        );
        if (curval.indexOf("</a>") != -1) {
          curlink = jQuery(Iam.handsOnTab.getCell(selected[0], selected[1]))
            .find("a")
            .attr("href");
        }
      ...
      if (jQuery(this).attr("id") == "jteditfont") {
        var selectopt =
          Iam.handsOnTab.getCellMeta(selected[0], selected[1])["jtcellstyle"] &&
          Iam.handsOnTab.getCellMeta(selected[0], selected[1])["jtcellstyle"][
            "font-family"
          ] != undefined
            ? Iam.handsOnTab.getCellMeta(selected[0], selected[1])[
                "jtcellstyle"
              ]["font-family"]
            : "Inherit";
        var selectopt2 =
      ...
      if (jQuery(this).attr("id") == "jthidecolsbtn") {
        var selectopt = Iam.handsOnTab.getCellMeta(0, selected[1])[
          "jtfootablebps"
        ]
          ? Iam.handsOnTab.getCellMeta(0, selected[1])["jtfootablebps"]
          : {
              xsmall: "",
              small: "",
              medium: "",
        ...
        bpopts.each(function (element, uindx) {
          ...
          var thisvarvaljt = thisElem.data("footab-hidden");
        ...
        var headerBreakPoints =
          Iam.handsOnTab.getCellMeta(0, selected[1])["jtfootabcoltype"] ||
          "text";
  ...
  jQuery(".jtrt-toolbar-more, .cp-color-picker").on(
    "click",
    "ul",
    function (elem) {
      elem.preventDefault();
  ...
  jQuery("#jteditfont ul button#jtsetfontcolorbtn").on(
    "click",
    function (elem) {
  ...
  jQuery("#jthighlight ul button#jtsetcellcolorbtn").on(
    "click",
    function (elem) {
  ...
  jQuery(".jtalignbtn").on("click", function () {
    var vall = jQuery(this).data("align-opt");
  ...
  jQuery("#jtsetcellbordersbtn").on("click", function () {
    var selectedBorder = jQuery("#jtbordersbtn .borderSelector .active").map(
      function (indx, elem) {
        return jQuery(elem).data("border");
      },
    );
  ...
  jQuery("#jtinputvalbox").on("keyup", function () {
    ...
    if (edittingRow != undefined && edittingCol != undefined) {
      Iam.handsOnTab.setDataAtCell(
        parseInt(edittingRow),
        parseInt(edittingCol),
        jQuery(this).val(),
      );
    }
  });
    ...
    window.setTimeout(function () {
      ...
      if (selected != undefined && selected.length > 0) {
        Iam.generateSelectionFunc(selected, function (i, t) {
          var headerBreakPoints = Iam.handsOnTab.getCellMeta(0, t)[
            "jtfootablebps"
          ] || {
            xsmall: "",
            small: "",
...
JTrtEditor.prototype.getData = function () {
  if (this.dataBox.html() !== "") {
    ...
    return [
      jtrt_saved_data[0],
      jtrt_saved_data[1] || {},
      jtrt_saved_data[2].length ? jtrt_saved_data[2] : true,
    ];
  } else {
    return [
      [
...
JTrtEditor.prototype.safeHtmlRenderer = function (
  instance,
  td,
  row,
  col,
  ...
  if (
    cellProperties["jtcellstyle"] != undefined &&
    cellProperties["jtcellstyle"]["font-family"] != undefined
  ) {
    td.style.fontFamily = cellProperties["jtcellstyle"]["font-family"];
  }
  if (
    cellProperties["jtcellstyle"] != undefined &&
    cellProperties["jtcellstyle"]["font-size"] != undefined
  ) {
    jQuery(td).css("font-size", cellProperties["jtcellstyle"]["font-size"]);
  }
  if (
    cellProperties["jtcellstyle"] != undefined &&
    cellProperties["jtcellstyle"]["font-weight"] != undefined
  ) {
    jQuery(td).css("font-weight", cellProperties["jtcellstyle"]["font-weight"]);
  }
  if (
    cellProperties["jtcellstyle"] != undefined &&
    cellProperties["jtcellstyle"]["font-style"] != undefined
  ) {
    jQuery(td).css("font-style", cellProperties["jtcellstyle"]["font-style"]);
  }
  if (
    cellProperties["jtcellstyle"] != undefined &&
    cellProperties["jtcellstyle"]["text-decoration"] != undefined
  ) {
    jQuery(td).css(
      "text-decoration",
      cellProperties["jtcellstyle"]["text-decoration"],
    );
  }
  if (
    cellProperties["jtcellstyle"] != undefined &&
    cellProperties["jtcellstyle"]["color"] != undefined
  ) {
    jQuery(td).css("color", cellProperties["jtcellstyle"]["color"]);
  }
  if (
    cellProperties["jtcellstyle"] != undefined &&
    cellProperties["jtcellstyle"]["background"] != undefined
  ) {
  ...
  if (!Iam.dataSaved) {
    ...
    JSON.parse(tableDataJT).forEach(function (emel, dataRowIndex) {
      emel.forEach(function (colEmel, dataColIndex) {
        if (colEmel != null && colEmel[0] === "=") {
          var calculatedCell = jQuery(
            Iam.handsOnTab.getCell(dataRowIndex, dataColIndex),
          ).html();
          var funcCellData = {
            row: dataRowIndex,
            col: dataColIndex,
    ...
    Iam.dataBox.html(
      "[" +
        tableDataJT +
        "," +
        JSON.stringify(tableCellDataNew) +
        "," +
        JSON.stringify(tableCellBorderData) +
        "," +
        JSON.stringify(tableFuncResData) +
        "]",
    );
    ...
    if (selected != undefined && selected.length > 0) {
      if (opt == "className") {
        ...
        Iam.generateSelectionFunc(selected, function (i, t) {
          ...
          if (curval.indexOf("</a>") != -1) {
            var curlink = jQuery(Iam.handsOnTab.getCell(i, t))
              .find("a")
              .attr("href");
            var newvl = Iam.strip_tags(
              curval,
              "<em><b><strong><u><big><img><i><br><caption><figure><span><hr><ul><li><dl><dd><dt><form><input><div><select><option>",
            );
            Iam.handsOnTab.setDataAtCell(
              i,
              t,
        ...
        Iam.generateSelectionFunc(selected, function (i, t) {
          ...
          jQuery('div[class^="wtBorder border_row' + i + "col" + t + '"')
            .parent()
            .remove();
        ...
        Iam.generateSelectionFunc(selected, function (i, t) {
          var currentMeta =
            Iam.handsOnTab.getCellMeta(i, t)["jtcellstyle"] || {};
          var vall = "inherit";
      ...
      element.forEach(function (element2, td) {
        if (element2.indexOf(args[0]) != -1) {
          Iam.handsOnTab.setDataAtCell(
            row,
            td,
            element2.replace(args[0], args[1]),
          );
        }
      }, this);
    }, this);
...
JTrtEditor.prototype.insertStuff = function (type, elem) {
  ...
  window.setTimeout(function () {
    ...
    if (selected != undefined && selected.length > 0) {
      if (typeStuff[0] == "row") {
        Iam.handsOnTab.alter(
          "insert_row",
          selected[0] + parseInt(typeStuff[1]),
        );
      } else if (typeStuff[0] == "col") {
        Iam.handsOnTab.alter(
          "insert_col",
          selected[1] + parseInt(typeStuff[1]),
        );
      }
    } else {
      alert("You have to first select cells that you want to edit");
