(function ($) {
  ...
  $(function () {
    jQuery("ul#jt-steps li").on("click", function () {
      var thisid = jQuery(this).data("jtrt-editor-section-id");

      if (
        jQuery(".jtrteditorsection#step" + thisid).hasClass("jtrteditorshow")
      ) {
        return false;
      }
      ...
      var currentShownSection = jQuery(
        ".jtrteditorsection.jtrteditorshow",
      ).fadeOut();
      window.setTimeout(function () {
        currentShownSection.removeClass("jtrteditorshow");
        jQuery(".jtrteditorsection#step" + thisid)
          .fadeIn()
          .addClass("jtrteditorshow");
      }, 400);
    });
    ...
    jQuery(".leftSidejt ul li").on("click", function () {
      ...
      if (
        jQuery(".rightSidejt #optionsSection" + thisid).hasClass(
          "jtoptionsshow",
        )
      ) {
        return false;
      ...
      window.setTimeout(function () {
        ...
        jQuery(".rightSidejt #optionsSection" + thisid)
          .fadeIn()
          .addClass("jtoptionsshow");
      }, 400);
    });

    JTrtTableEditor = new JTrtEditor(
      document.getElementById("jtrt-table-handson"),
    );
    ...
    jQuery("#jtundo,#jtredo").on("click", function () {
      JTrtTableEditor.rudo($(this).data("jtrt-btnType"));
    });
    ...
    $(
      "#normal-sortables, #jtrt_tables_post .hndle, #jtrt_tables_post .handlediv",
    ).remove();
    $("#jtrt_tables_post").prepend("<h1>JT Responsive Tables</h1>");

    $(
      '#jtresponsiveoptionscontainer div[data-jtresponsive-select="' +
        $("#jtresponsiveoptionscontainerselect").val() +
        '"]',
    ).fadeIn();
    ...
    function makeDBcall(data) {
      jQuery.post(ajaxurl, data, function (response) {
        if (response == false) {
          alert(
            "This message will no longer appear. Thank you for using JTRT plugin!",
          );
          $("#jtConverAvailMessage").fadeOut();
        } else {
          var tableHolder = $("<div></div>").html(
            response.substring(0, response.length - 1),
          );
          var newData = [];
          tableHolder
            .find("table tr:not(.jtrt_custom_header)")
            .each(function (indx, val) {
              var thisRow = [];
              $(val)
                .find("td:not(.jtrt_custom_td)")
                .each(function (colind, colval) {
    ...
    jQuery(".submitbox").append(
      '<div style="text-align:center;padding:14px;"><strong>Shortcode</strong><br/><span style="padding:8px">[jtrt_tables id="' +
        jQuery("#post_ID").val() +
        '"]</span></div>',
    );
  }); // .ready
